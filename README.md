...
```bash
bash run.sh        # 首次启动自动初始化演示数据，约 8 秒
```

打开 http://localhost:8080。演示数据自动就位：4 笔覆盖全状态的订单——**张伟（今日入住待办理）/ 李娜（在住）/ 王强（今日退房）/ 赵敏（待支付）**，3 个房型 + 未来 60 天房量/价格日历 + 1 条 LOS 收益管理规则。

### 第二步：主流程（3 分钟）

| # | 页面 | 操作 | 预期效果（讲给评委听） |
|---|---|---|---|
| 1 | 预订房间 | 选日期（跨周末可看到**周末价上浮 20%**）→ 查询房态 → 选房型 → 填入住人 → 确认预订 | 订单号 HB 开头、状态"待支付"；金额 = Σ每晚价 × 间数 |
| 2 | 预订房间 | 结果卡点"模拟支付" | 状态变"已确认" |
| 3 | 订单查询 | 输入手机号 13800138001 → 点开张伟订单 | 状态时间线：提交预订 → 模拟支付成功 |
| 4 | 前台管理 | 张伟订单点"办理入住" | 状态"已入住"，统计卡（今日入住/在住/入住率）实时变化 |
| 5 | 前台管理 | （可选）李娜订单"办理退房" | 全状态闭环，今日退房 +1 |

### 第三步：技术亮点现场演示（每个都是得分点，2 分钟）

1. **防超卖**：某房型连续下单至满房 → 卡片置灰、强制提交被"房量不足"拒绝；再现场跑 `mvn -Dtest=ConcurrencyTest test`（悲观锁：10 线程抢 1 间恰 1 成功）与 `mvn -Dtest=OptimisticInventoryTest test`（**乐观锁同规格证明**——双模式）
2. **幂等**：双击"确认预订"只产生一单（前端防抖 + request_id 唯一索引双层防护）
3. **LOS 收益管理**：选大床房、日期 +8~+10 天、住 1 晚 → 被拒"需连住至少 2 晚"；住 2 晚放行
4. **状态机**：对已退房订单再办理入住 → 409"订单当前状态为「已退房」，不允许执行该操作"
5. **API 契约**：打开 /swagger-ui.html，11 个接口在线调试

### 现场救急（常见问题）

| 问题 | 解决 |
|---|---|
| 端口 8080 被占（如 IDEA 里跑过旧实例） | 停掉旧进程，或 `--server.port=8081` |
| 演示数据想重置 | `rm -rf data && bash run.sh`（数据自动重建） |
| 评委想看真实 MySQL | `docker compose up -d` 后以 `--spring.profiles.active=mysql` 启动 |
